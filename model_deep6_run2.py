...
if __name__ == "__main__": 
    ...
    layers = [4, 50, 50, 50, 50, 50, 50, 7] # first layer should be size of X in the PINN
...
if __name__ == "__main__": 
    ...
    # relative RMSE (rescaled back to physical units)   
    error_T = np.sqrt(np.mean(np.square(char_train[4]*(T_test - T_pred))))/np.mean((T_test+1)*char_test[4] + lb_test[4])
    error_S = np.sqrt(np.mean(np.square(char_train[5]*(S_test - S_pred))))/np.mean((S_test+1)*char_test[5] + lb_test[5])
    error_u = np.sqrt(np.mean(np.square(char_train[6]*(u_test - u_pred))))/np.mean((u_test+1)*char_test[6] + lb_test[6])
    error_v = np.sqrt(np.mean(np.square(char_train[7]*(v_test - v_pred))))/np.mean((v_test+1)*char_test[7] + lb_test[7])
    error_w = np.sqrt(np.mean(np.square(char_train[8]*(w_test - w_pred))))/np.mean((w_test+1)*char_test[8] + lb_test[8])
    # error_w = np.sqrt(np.mean(np.square((w_test+1)*char_test[8] + lb_test[8] - w_pred)))/np.mean((w_test+1)*char_test[8] + lb_test[8]) # altered because no training data for w
    error_p = np.sqrt(np.mean(np.square(char_train[9]*(p_test - p_pred))))/np.mean((p_test+1)*char_test[9] + lb_test[9])
    error_rho = np.sqrt(np.mean(np.square(char_train[10]*(rho_test - rho_pred))))/np.mean((rho_test+1)*char_test[10] + lb_test[10])

    ## SAVING THE MODEL
    ckpt_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/model-depth/latlon_k2037_j4070_i4070_gamma10_iter16000_layer50_deep6_run2.ckpt"
    array_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/model-depth/latlon_k2037_j4070_i4070_gamma10_iter16000_layer50_deep6_run2.npz"

    model.save_model(ckpt_file)
    np.savez(array_file, inputs_train=inputs_train, char_train=char_train, lb_train=lb_train, col_train = col_train, f_col = f_col, inputs_val = inputs_val, coords_val = coords_val, inputs_test = inputs_test, char_test = char_test, lb_test = lb_test, subset_template = subset_template, layers=layers, coords_test=coords_test, data_losses=data_losses, eq_losses=eq_losses, val_losses = val_losses, losses=losses)
