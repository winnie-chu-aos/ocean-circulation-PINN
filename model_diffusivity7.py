...
with dask.config.set(**{'array.slicing.split_large_chunks': True}):
    data_path = '/scratch/gpfs/wc4720/SOCCOM/OCEAN_BOLUS_STREAMFUNCTION_day_mean_1992-01_ECCO_V4r4_latlon_0p50deg.nc'
    # data_path = '/projects/SOCCOM/datasets/ecco/Version4/Release4/interp_monthly/'+tag+'/' + str(i) +'/'+tag+'_' + str(i) + '_??.nc'
    ds = xr.open_mfdataset(data_path,chunks="auto",data_vars='minimal',coords='minimal', compat='override') 
    
ds2 = xr.Dataset()

tags = ['TEMPERATURE_SALINITY', 'VELOCITY']

for tag in tags:
    with dask.config.set(**{'array.slicing.split_large_chunks': True}):
        data_path = '/projects/CDEUTSCH/DATA/OCEAN_'+ tag + '_day_mean_1992-01-??_ECCO_V4r4_latlon_0p50deg.nc'
        # data_path = '/projects/SOCCOM/datasets/ecco/Version4/Release4/interp_monthly/'+tag+'/' + str(i) +'/'+tag+'_' + str(i) + '_??.nc'
        dsnow = xr.open_mfdataset(data_path,chunks="auto",data_vars='minimal',coords='minimal', compat='override') 
        ds2 = xr.merge([ds2, dsnow],compat='override')
...
ds2 = ds2.rename({'WVEL': 'WVELMASS_interp'})
...
Kz = ds.KAPREDI*(Smag**2) + ds.DIFFKR
...
ds = xr.merge([ds, maskC.rename('maskC')],compat='override')

ds['time'].astype('datetime64[h]').values.astype('int64')// 10**9 # get the time steps in number form
ds['timestep'] = (('time'), (ds['time'].values - ds['time'].values[0]) / np.timedelta64(1, 'h'))  # convert to hours

del Sx, Sy, Smag, Kx, Ky, Kz, maskC
...
tags = ['THETA','SALT','EVEL', 'NVEL', 'WVELMASS_interp']
...
# SAVING THE MODEL
ckpt_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/diffusivity-noprho/latlon_k2040_j4090_i300350_iter2500_layer20_gamma10_omitvel3.ckpt"
array_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/diffusivity-noprho/latlon_k2040_j4090_i300350_iter2500_layer20_gamma10_omitvel3.npz"
