...
ds = xr.Dataset()
tags = ['THETA','SALT','EVEL', 'NVEL', 'WVELMASS', 'TFLUX', 'SFLUX']
...
# PROCESSING THE HEAT AND SALT FLUX
test = ds.TFLUX/4000/1000/5 # 4000 J/kg/K specific heat, 1000 kg/m^3 density, 5 m depth of surface layer
test2 = np.tile(test,(np.size(ds.k),1,1,1))
test2[1:,:,:,:] = 0
ds['TFLUX'] = (('time','k','j', 'i'), test2.swapaxes(0,1))

test = ds.SFLUX/1000/1000/5 # 1000 kg/m^3 density, 1/1000 kg/g, 1 g/1000 g PSU, 5 m depth of surface layer
test2 = np.tile(test,(np.size(ds.k),1,1,1))
test2[1:,:,:,:] = 0
ds['SFLUX'] = (('time','k','j', 'i'), test2.swapaxes(0,1))
...
# VERSION WITH THE DIFFUSIVITY AS TENSORFLOW VARIABLES NOT OUTPUTS (NO FLUX)
class PhysicsInformedNN:
    ...
    def __init__(self, inputs_nn, char_nn, lb_nn, col_nn, val_nn, layers, f_nn, gamma):
        ...
        self.v_c = col_nn[:,7:8]
        self.w_c = col_nn[:,8:9]
        self.Tflux_c = col_nn[:,9:10]
        self.Sflux_c = col_nn[:,9:10]
        ...
        self.Tflux_ctf = tf.placeholder(tf.float32, shape=[None, self.Tflux_c.shape[1]])
        self.Sflux_ctf = tf.placeholder(tf.float32, shape=[None, self.Sflux_c.shape[1]])
        ...
        # self.Kz = tf.Variable(initial_value=1e-5*tf.ones(shape=self.i_c.shape, dtype=tf.float32))
        # self.Ky = tf.Variable(initial_value=1e3*tf.ones(shape=self.i_c.shape, dtype=tf.float32))
        # self.Kx = tf.Variable(initial_value=1e3*tf.ones(shape=self.i_c.shape, dtype=tf.float32))
        self.Kz = tf.Variable(initial_value=tf.zeros(shape=self.i_c.shape, dtype=tf.float32))
        self.Ky = tf.Variable(initial_value=tf.zeros(shape=self.i_c.shape, dtype=tf.float32))
        self.Kx = tf.Variable(initial_value=tf.zeros(shape=self.i_c.shape, dtype=tf.float32))

        # training points
        self.T_pred, self.S_pred = self.net_NS(self.t_tf, self.k_tf, self.j_tf, self.i_tf)

        # validation points
        self.T_val_pred, self.S_val_pred = self.net_NS(self.t_vtf, self.k_vtf, self.j_vtf, self.i_vtf)
        ...
        self.eqloss = self.fT_pred + self.fS_pred
                    # tf.reduce_mean(tf.square(self.fcont)) + \
        ...
        self.valloss = tf.reduce_mean(tf.square(self.T_vtf - self.T_val_pred)) + \
                    ...
                    # tf.reduce_mean(tf.square(self.u_vtf - self.u_val_pred)) + \
    ...
    def net_f_NS(self, t, k, j, i):
        ...
        v_hat = self.v_c
        w_hat = self.w_c
        Tflux_hat = self.Tflux_c
        Sflux_hat = self.Sflux_c
        # w_full = self.w_test
        ...
        Tflux_full = (Tflux_hat+1)*char_nn[9]+lb_nn[9]
        Sflux_full = (Sflux_hat+1)*char_nn[10]+lb_nn[10]
        # rho_full = (rho_hat+1)*char_nn[10]+lb_nn[10]
    
        # temperature conservation
        fT = char_nn[3]/char_nn[4]*(char_nn[4]/char_nn[0]*T_t + char_nn[4]/char_nn[3]*u_full*T_x + char_nn[4]/char_nn[2]*v_full*T_y + char_nn[4]/char_nn[1]*w_full*T_z - Kx*char_nn[4]/char_nn[3]**2*T_xx - Ky*char_nn[4]/char_nn[2]**2*T_yy - Kz*char_nn[4]/char_nn[1]**2*T_zz - Tflux_full) 
        
        # salinity conservation
        fS = char_nn[3]/char_nn[5]*(char_nn[5]/char_nn[0]*T_t + char_nn[5]/char_nn[3]*u_full*S_x + char_nn[5]/char_nn[2]*v_full*S_y + char_nn[5]/char_nn[1]*w_full*S_z - Kx*char_nn[5]/char_nn[3]**2*S_xx - Ky*char_nn[5]/char_nn[2]**2*S_yy - Kz*char_nn[5]/char_nn[1]**2*S_zz - Sflux_full) 
...
# SAVING THE MODEL
ckpt_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/diffusivity-noprho/latlon_k020_j4090_i300350_iter2500_layer20_gamma10_omitvel6.ckpt"
array_file = "/scratch/gpfs/wc4720/SOCCOM/saved-models/diffusivity-noprho/latlon_k020_j4090_i300350_iter2500_layer20_gamma10_omitvel6.npz"
